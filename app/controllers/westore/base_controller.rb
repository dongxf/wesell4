class Westore::BaseController < InheritedResources::Base
  layout 'westore'

  # before_filter :check_user_agent
  #before_filter :parse_query, except: [:order_status, :update_order, :choose_express]
  before_filter :track_logger_b
  before_filter :parse_query
  before_filter :authenticate_openid!
  after_filter :track_logger_a

  ##need new namespace for manager by fwdesk?
  ...
  def track_logger_b; logger.info "==== westore base before: instance s#{session[:instance_id_4]} c#{current_instance.try(:id)}, order c#{current_order.try(:id)}, store s#{session[:store_id_4]} c#{current_store.try(:id)} customer s#{session[:customer_id_4]} c#{current_customer.try(:id)} ====" end
  def track_logger_a; logger.info "==== westore base after: instance s#{session[:instance_id_4]} c#{current_instance.try(:id)}, order c#{current_order.try(:id)}, store s#{session[:store_id_4]} c#{current_store.try(:id)} customer s#{session[:customer_id_4]} c#{current_customer.try(:id)} ====" end
  ...
  def parse_query
    ...
    if params[:id]
      ...
      when 'westore/order_items'
        ...
        @current_order ||= oi.order if oi && oi.order
        @current_store ||= @current_order.store if @current_order && @current_order.store
      ...
      when 'westore/wesell_items'
        ...
        @current_instance ||= @current_store.instances.first if @current_store && @current_store.instances.first
      when 'westore/kategories'
        kt = Kategory.find_by id: tid
        @current_instance ||= kt.instance if kt && kt.instance
      else
        logger.info "==== westore params id parse nil===="
      end
    end
    ...
    @current_instance ||= Instance.find_by id: session[:instance_id_4] if session[:instance_id_4]

    logger.info "tbd2: instance#{@current_instance.try(:id)} store#{@current_store.try(:id)} order#{@current_order.try(:id)}"

    if @current_instance.blank?
      @no_bottom = true
  ...
  def authenticate_openid!
    ...
    logger.info("==== oauth start: instance #{current_instance.try(:id)} ====")
    ...
    if session[oidkey].blank?
      logger.info("==== oauth oidkey is blank  ====")
      code = params[:code]
    # 如果code参数为空，则为认证第一步，重定向到微信认证
      ...
      logger.info("==== oauth oidkey #{session[oidkey]} ====")
    end
    ...
    if !customer.present?
      ...
      logger.info("==== oauth: create new customer: #{customer.try(:id)} ====")
    else
      logger.info("==== oauth: restore customer #{customer.try(:id)} from oidkey====")
    end
  ...
  def current_customer
    ...
    if customer.present?; return set_current_customer(customer) else logger.info "==== restore customer failed  #{@current_instance.try(:id)} #{session[:customer_id_4]} ====" end
