module Platform::OrdersHelper
  def order_status order, status, options = {}
    case status
    when :reject
      if order.status == 'rejected'
        link_to '已拒绝', 'javascript:void(0)', class: 'btn btn-default'
      else
        link_to '拒绝', platform_order_path(order, event: 'reject'), method: 'PATCH', \
    ...
    when :accept
      if order.status == 'accepted'
        ...
        link_to '接受', platform_order_path(order, event: 'accept'), method: 'PATCH', \
    ...
    when :ship
      if order.status == 'shipped'
        ...
        link_to '发货', platform_order_path(order, event: 'ship'), method: 'PATCH', \
    ...
    when :finish
      if order.status == 'finished'
        ...
        link_to '完成订单', platform_order_path(order, event: 'finish'), method: 'PATCH', \
